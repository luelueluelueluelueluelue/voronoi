...
bool Beachline::isNil(Arc* x)
{
	return x == mNil || (unsigned int)x == 0xCDCDCDCD;
}
...
Arc* Beachline::locateArcAbove(const Vector2& point, double l)
{
	Arc* node = mRoot;
	bool found = false;
	while (!found)
	{
		...
		if (!isNil(node->prev))
		{
			if (std::abs(node->prev->site->point.y - node->site->point.y) < 0.0000001
				&& node->site->point.x > node->prev->site->point.x)
			{
				breakpointLeft = (node->site->point.x + node->prev->site->point.x) / 2.0;
				breakpointRight = std::numeric_limits<double>::infinity();
			}
			else
			{
				breakpointLeft = computeBreakpoint(node->prev->site->point, node->site->point, l);
				if (breakpointLeft == std::numeric_limits<double>::infinity())
				{
					breakpointLeft = -std::numeric_limits<double>::infinity();
				}
			}
		}
		if (!isNil(node->next) && breakpointRight == std::numeric_limits<double>::infinity())
		{
			if (std::abs(node->next->site->point.y - node->site->point.y) < 0.0000001
				&& node->site->point.x < node->next->site->point.x)
			{
				breakpointRight = (node->site->point.x + node->next->site->point.x) / 2.0;
				breakpointLeft = -std::numeric_limits<double>::infinity();;
			}
			else
			{
		...
		if (point.x < breakpointLeft)
			...
			if (std::abs(point.y - node->site->point.y) < 0.0000001)
				if (std::abs(point.y - 600.00) < 0.0000001)
				{
					if (point.x < node->site->point.x)
					{
						found = true;
					}
					else
					{
						if (!isNil(node->left) && point.x < node->left->site->point.x)
						{
							node = node->left;
						}
						else if (!isNil(node->right) && point.x < node->right->site->point.x)
						{
							node = node->right;
						}
						else
				...
				{
					if (!isNil(node->left) && point.x < node->site->point.x)
					{
						node = node->left;
					}
					else if (!isNil(node->right) && point.x < node->site->point.x)
					{
						node = node->right;
					}
					else
				...
				/*
				if (!isNil(node->left) && point.x < node->site->point.x)
					if (std::abs(node->left->site->point.y - node->site->point.y) < 0.0000001)
						found = true;
					else
						node = node->left;
				else if (!isNil(node->right) && point.x > node->site->point.x)
					if (std::abs(node->right->site->point.y - node->site->point.y) < 0.0000001)
						found = true;
					else
						node = node->right;
				else
	...
	/*
	if (!isNil(node->prev) && !isNil(node->next) && node->prev->site->point.y == node->site->point.y)
	{
		if (std::abs(point.x - node->prev->site->point.x) < std::abs(point.x - node->site->point.x))
		{
			node = node->left;
		}
		else
...
std::ostream& Beachline::print(std::ostream& os)
{
...
double Beachline::computeBreakpoint(const Vector2& point1, const Vector2& point2, double l)
{
...
std::ostream& Beachline::printArc(std::ostream& os, const Arc* arc, std::string tabs)
{
